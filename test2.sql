...
--17. 스펙과 같은 번호~5 큰 번호의 아이템을 선택해 넣음
insert into bom
    select 'S' || lpad(s.no, 2, 0) spec_code, 
           'I' || lpad(i.no, 2, 0) item_code,
           CEIL(DBMS_RANDOM.VALUE(2,5)) item_qty
    from study01.t1_data s, study01.t1_data i
    where s.no <= 5 and i.no <= 10
      and i.no between s.no and s.no + 5
    order by 1;

--18. 매일 똑같이 돌아가는 공장(line1에 spec1 6개, spec2 4개 / line2에 spec2 2개, spec2 4개...
insert into input_plan
    select s.no plan_seq, 
           '2019100' || d.no  plan_date, 
           l.line_no,
           'S' || lpad(mod(ceil(rownum / 6) - 1, 5) + 1, 2, 0) spec_code
    from study01.t1_data s, line l, study01.t1_data d
    where s.no <= 10 and d.no <= 5;
...
insert into jit_delivery_plan
    select input_plan_date, 
           line_no, 
           ceil(plan_seq / 2) jit_seq, -- 1일 2회(1~5/6~10)
           item_code, 
           sum(item_qty),
           '' -- 배송여부
    from input_plan i, bom b
    where i.spec_code = b.spec_code
    group by (input_plan_date, line_no, ceil(plan_seq / 2), item_code)
    order by 1, 2, 3, 4;
commit;
