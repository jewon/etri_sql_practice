...
*/
select lpad(d.dept_code, length(d.dept_code) + level - 1, '*') as dept_code, 
       d.dept_name
from tdept d, tdept p
where d.parent_dept = p.dept_code(+)
start with d.dept_code = 'CEO001' 
connect by prior d.dept_name = p.dept_name;
...
select count(*) answer
from temp
where not ((nvl(hobby, 0) = '독서') or (nvl(hobby, 0) = '여행'));
...
-- 4.5
select /*+ INDEX_DESC(TEMP, SALARY1)*/emp_id, salary 
from temp
where salary > 0;
